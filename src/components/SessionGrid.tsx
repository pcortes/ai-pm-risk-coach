...
import { CooFocusPanel } from "./CooFocusPanel";
import { CooDashboardSummary, getCooBrief, getTopProjectFocus } from "@/lib/coo-advisor";
...
export function SessionGrid({
  ...
  cooSummary?: CooDashboardSummary;
  viewMode: ViewMode;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="text-zinc-600 text-sm mt-1.5 max-w-xs text-center leading-relaxed">
          Start a Claude Code or Codex session and it will appear here automatically.
        </p>
      </div>
    );
  }
  ...
  const focus =
    cooSummary?.focus ??
    getTopProjectFocus(sessions).map((item, index) => ({
      ...item,
      rank: index + 1,
      delta: null,
      compliance: {
  ...
  const briefMap = new Map(sessions.map((session) => [session.id, getCooBrief(session)]));
  const waitingCount = cooSummary?.waitingCount ?? sessions.filter((session) => session.status === "waiting").length;
  const reviewCount =
    cooSummary?.reviewCount ??
  ...
  const atRiskCount =
    cooSummary?.atRiskCount ?? sessions.filter((session) => briefMap.get(session.id)?.priority === "critical").length;
  ...
  for (const group of groups) {
    for (const session of group.sessions) {
      flatIndexMap.set(session.id, flatIdx);
      flatIdx++;
    }
  }
  ...
  const sendKeystrokeForRow = async (session: ClaudeSession, keystroke: string, action: "approve" | "reject") => {
    onApproveReject?.(session.id, action);
    try {
      const brief = getCooBrief(session);
      await sendKeystrokeAction(session.pid!, keystroke, {
        repoPath: session.parentRepo || session.workingDirectory,
    ...
    return (
      <SessionRow
        ...
        onApproveReject={
          session.pid
            ? (action) => {
                sendKeystrokeForRow(session, action === "approve" ? "return" : "escape", action);
              }
            : undefined
        }
      />
    ...
    <div className="space-y-8">
      <CooFocusPanel
        ...
        onSelectSessionKey={
          onSelectIndex
            ? (sessionKey) => {
                const idx = flatIndexMap.get(sessionKey);
                if (idx !== undefined) onSelectIndex(idx);
              }
            : undefined
        }
      />
