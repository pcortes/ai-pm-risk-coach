    ...
    try {
      const res = await fetch("/api/sessions/create", {
        ...
        body: JSON.stringify({
          ...
          tmuxSession: provider === "claude" ? selectedTmuxSession || undefined : undefined,
        }),
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          {!needsSetup && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["claude", "codex"] as SessionProvider[]).map((option) => (
                  <button
                    key={option}
                    type="button"
              ...
              <p className="mt-1 text-[11px] text-zinc-600">
                {provider === "claude"
                  ? "Launches a terminal-backed Claude session with full terminal controls."
                  : "Launches a managed Codex session through app-server so replies happen directly in the dashboard."}
              </p>
            </div>
          )}
          ...
          {!needsSetup &&
            provider === "claude" &&
            terminalConfig?.terminalUseTmux &&
            terminalConfig.terminalTmuxMode === "choose" && (
            <div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {loading ? (
                  ...
                  provider === "claude" ? "Create Worktree & Launch" : "Create Worktree & Start Codex"
                ) : (
                  provider === "claude" ? "Launch Claude" : "Start Codex"
                )}
              </button>
            </div>
          )}
