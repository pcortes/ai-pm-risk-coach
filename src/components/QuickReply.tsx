...
import {
  CooActionMeta,
  logCooOperatorAction,
  sendKeystrokeAction,
  sendMessageAction,
} from "@/lib/actions";
import type { SessionProvider, ToolInfo } from "@/lib/types";
...
export function QuickReply({
  ...
  const sendKeystroke = async (keystroke: string, label: string) => {
    ...
    try {
      const operatorAction: CooActionMeta["operatorAction"] = label === "approve" ? "approve" : "reject";
      const meta = draftMeta
        ...
        : undefined;
      await sendKeystrokeAction(pid, keystroke, meta);
    } catch (err) {
  ...
  const isPermissionPrompt = hasPendingToolUse && canSendKeystroke;
  const draftReply = () => {
    if (!suggestedReply) return;
    setMessage(suggestedReply);
    if (isPermissionPrompt) setShowReply(true);
    setTimeout(() => inputRef.current?.focus(), 100);
    if (draftMeta) {
      logCooOperatorAction({ ...draftMeta, operatorAction: "use-draft" }).catch(() => {});
    }
  };
  ...
  return (
    <div onClick={stopProp} onMouseDown={stopProp} className="mt-3 cleanup-slide-in">
      ...
      {suggestedReply && (
        <div className="mb-2.5 rounded-lg border border-cyan-500/14 bg-cyan-500/7 px-2.5 py-2">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-cyan-300/90">COO Draft</p>
              <p className="mt-1 text-[11px] leading-relaxed text-cyan-100/85 line-clamp-3">{suggestedReply}</p>
            </div>
            <button
              onClick={(e) => {
                stopProp(e);
              ...
              className="shrink-0 rounded-lg border border-cyan-500/18 bg-cyan-500/10 px-2 py-1 text-[10px] font-medium text-cyan-200 hover:bg-cyan-500/16 transition-colors"
            >
              Use draft
            </button>
          </div>
      ...
      {isPermissionPrompt ? (
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            onClick={stopProp}
            onFocus={stopProp}
            placeholder={sending === "reply" ? "Sent!" : "Reply to agent..."}
            disabled={sending !== null}
