...
import { sendManagedCodexMessage } from "@/lib/codex-app-server";
import {
  buildProcessTree,
  detectAllTmuxPanes,
  detectTerminal,
...
import { recordCooOperatorAction } from "@/lib/coo-state";
...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      normalizedAction !== "stage-message" &&
      normalizedAction !== "send-keystroke" &&
      normalizedAction !== "open-url" &&
      normalizedAction !== "log-operator-action"
    ) {
      if (!path) {
    ...
    switch (normalizedAction) {
      case "focus": {
        ...
        if (body.repoPath) {
          await recordCooOperatorAction({
            repoPath: body.repoPath,
            repoName: body.repoName,
            sessionKey: body.sessionKey,
            fingerprint: body.fingerprint,
            actionType: body.operatorAction ?? "focus",
          });
        }
        break;
      }
      ...
      case "send-message": {
        ...
        if (body.provider === "codex") {
          if (!body.providerSessionId) {
            return NextResponse.json({ error: "Missing providerSessionId for Codex message" }, { status: 400 });
          }
          await sendManagedCodexMessage({
            threadId: body.providerSessionId,
            message,
          });
        } else {
          if (!pid) {
        ...
        if (body.repoPath) {
          await recordCooOperatorAction({
            ...
            sessionKey: body.sessionKey,
            fingerprint: body.fingerprint,
            actionType: body.operatorAction ?? "reply",
          });
        }
        break;
      }
      case "stage-message": {
        if (!pid) {
          return NextResponse.json({ error: "Missing pid for stage-message action" }, { status: 400 });
        }
        if (!message) {
          return NextResponse.json({ error: "Missing message" }, { status: 400 });
        }
        ...
        if (body.repoPath) {
          await recordCooOperatorAction({
            ...
            sessionKey: body.sessionKey,
            fingerprint: body.fingerprint,
            actionType: body.operatorAction ?? "load-draft",
          });
        }
        break;
      }
      case "send-keystroke": {
        ...
        if (body.repoPath) {
          await recordCooOperatorAction({
            ...
            fingerprint: body.fingerprint,
            actionType:
              body.operatorAction ?? (keystroke === "return" ? "approve" : keystroke === "escape" ? "reject" : "focus"),
          });
        }
        break;
      }
...
tell application "${browserDef.appName}"
  ...
  if not found then
    ...
    open location "${escapedUrl}"
      ...
      case "log-operator-action": {
        if (!body.repoPath || !body.operatorAction) {
          return NextResponse.json({ error: "Missing repoPath or operatorAction" }, { status: 400 });
        }
        await recordCooOperatorAction({
          repoPath: body.repoPath,
          repoName: body.repoName,
          sessionKey: body.sessionKey,
          fingerprint: body.fingerprint,
          actionType: body.operatorAction,
        });
        break;
      }
      default:
