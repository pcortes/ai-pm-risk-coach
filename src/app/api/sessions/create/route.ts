...
import { startManagedCodexSession } from "@/lib/codex-app-server";
...
export async function POST(request: Request) {
  try {
    ...
    const { repoPath, branchName, baseBranch, prompt, tmuxSession, provider = "claude" } = body;
    ...
    if (provider === "codex") {
      const session = await startManagedCodexSession({
        cwd: targetPath,
        prompt,
      });
      ...
      return NextResponse.json({
        ...
        provider,
        path: targetPath,
        sessionId: `codex:${session.threadId}`,
    ...
    return NextResponse.json({ ok: true, provider, path: targetPath });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
