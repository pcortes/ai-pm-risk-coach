...
import { CLAUDE_SESSION_CAPABILITIES, ClaudeSession, ConversationPreview } from "./types";
...
import { discoverCodexSessions, getCodexSessionDetail } from "./codex-discovery";
...
async function discoverClaudeSessions(): Promise<ClaudeSession[]> {
...
export async function discoverSessions(): Promise<ClaudeSession[]> {
  const [claudeSessions, codexSessions] = await Promise.all([discoverClaudeSessions(), discoverCodexSessions()]);
