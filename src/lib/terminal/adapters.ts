...
export async function stageText(info: TerminalInfo, text: string): Promise<void> {
  if (info.inTmux && info.tmux) {
    await execFileAsync("tmux", ["set-buffer", "--", text], { timeout: PROCESS_TIMEOUT_MS });
    await execFileAsync("tmux", ["paste-buffer", "-t", info.tmux.paneId], { timeout: PROCESS_TIMEOUT_MS });
    return;
  }

  const adapter = getAdapter(info.app);
  await adapter.stageText(info, text);
}
